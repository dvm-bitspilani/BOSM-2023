...
const Navbar = ({
  ...
  const goToNextPage = () => {
    ...
    const handleAssetLoad = () => {
      ...
      if (assetsLoaded === assets.length) {
        setTimeout(() => {
          document.getElementsByClassName("blackScreen")[0].style.transform =
            "translateY(-100%)";
        }, 2000);
        setTimeout(() => {
          setShowBlackScreen(false);
  ...
  const navbarUpVariants = {
    ...
    closed: {
      y: -150,
      transition: {
        delay: isBrowser && window.innerWidth > 711 ? 0 : 0,
        duration: 1,
        ease: "easeInOut",
      },
    },
  ...
  const openHam = () => {
    ...
    if (
      isHamOpen &&
      (isAnimationComplete || (isBrowser && window.innerWidth < 711))
    ) {
      setIsHamOpen(false);
    } else {
      setIsHamOpen(true);
